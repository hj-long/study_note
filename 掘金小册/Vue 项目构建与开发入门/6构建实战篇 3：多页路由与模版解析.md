...
上篇文章中我们成功打包并输出了多页文件，而构建一个多页应用能够让我们进一步了解项目配置的可拓展性，可以对学习 Vue 和 webpack 起到强化训练的效果，本文将在此基础上主要针对多页路由及模版的配置进行系列的介绍。
...
上篇文章我们已经介绍了关于多模版的读取和配置，在配置 html-webpack-plugin 的时候我们提到了自定义配置，这里我将结合模版渲染的功能来进行统一介绍。

### 1. 模版渲染

这里所说的模版渲染是在我们的 html 模版文件中使用 html-webpack-plugin 提供的 [default template](https://github.com/jaketrent/html-webpack-template/blob/86f285d5c790a6c15263f5cc50fd666d51f974fd/index.html) 语法进行模版编写，比如：
...
```
以上我们使用模版语法手动获取并遍历 htmlWebpackPlugin 打包后的文件并生成到模版中，其中的 `htmlWebpackPlugin` 变量是模版提供的可访问变量，其有以下特定数据：
...
需要注意的是如果你在模版中编写了插入对应 js 及 css 的语法，你需要设置 `inject` 的值为 false 来关闭资源的自动注入：
...
在模版渲染中，我们只能够使用 htmlWebpackPlugin 内部的一些属性和方法来进行模版的定制化开发，那么如果遇到需要根据不同环境来引入不同资源，同时不同模版间的配置还可能不一样的需求情况的话，我们使用自定义配置会比较方便。比如我们需要在生产环境模版中引入第三方统计脚本：
...
同时你也可以针对个别模版进行配置，比如我想只在 Index 单页中添加统计脚本，在 Page1 单页中添加其他脚本，那么你可以给 addScript 传入标识符来进行判断输出，比如：
...
这样我们就完成了自定义配置中的模版渲染功能。当然根据实际项目需求你的自定义配置项可能会更加复杂和灵活。

## 结语

通过 2 小节的学习，相信大家对 Vue 多页应用的构建已经有所了解。本文在第 1 节的基础上重点介绍了多页路由及模版的配置，阐述了其与单页应用的不同之处，同时针对模版自定义配置的使用场景给出了简单的实例，希望大家在了解的基础上将下方的实例代码作为参考，进行相应的实战。
...
* html-webpack-plugin 如何解析非 .html 的模版，比如 .hbs，应该如何配置？
