...
在js中，对象是引用数据类型，也就是说如果 data 是一个对象时，每个组件实例的data对象引用的都是同一个内存地址，
一个实例数据改变了，其他的data也会收到影响跟着改变；

而如果 data 是一个函数，它会创建一个函数作用域作为自己的私有域，每个组件实例的data都是独立的，互不影响，
可以保证组件的一个正常工作。

## 9、数据双向绑定的原理

vue 中实现的这个数据双向绑定，首先要对数据进行劫持监听，用到了 Object.definedProperty() 的 get 和 set 方法，
...
axios 是一个基于 promise 的网络异步请求库，我们可以通过对它二次封装方便我们使用：

新建 utils/request.js ，导入 axios，可以设置 baseURL、timeOut超时时间等一些基础配置项，

然后还可以封装一层请求和响应各封装一层拦截器：
...
然后可以根据需要去调用，也可以使用 ts 类型去约束请求需要的参数类型，方便我们使用

例如 获取个人信息的接口、查询字典的接口 可以封装成一个个api函数，方便代码复用，例如我之前的项目中
有这个用户管理页它需要查询字典这个接口去查询该页面的用户是否有这个 启用 的功能，然后还有角色管理同样
也要用到这个 查询字典的 接口查询角色是否可以启用，因此这两个页面就可以直接引入api接口函数来使用就可以了
比较方便嘛
